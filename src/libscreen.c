...
Area *screen_area_init(int x, int y, int width, int height){
  ...
  if ((characters = (char*) malloc(LIB_SIZE * height *(width + 1)*sizeof(char))) == NULL){
    free(area);
    return NULL;
  }

  if ((area->character_array = (char**) malloc(height*sizeof(char*))) == NULL){
    free(characters);
    return NULL;
  }
